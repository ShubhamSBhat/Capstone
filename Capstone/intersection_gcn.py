...
def custom_action_policy(node_embeddings, vehicle_ids):
    for idx, id in enumerate(vehicle_ids):
        ...
        # Using the first dimension of the embedding as an example
        score = node_embeddings[idx][0]
        if score > 0.5:
...
cur_dir = os.getcwd()
sumo_config_path = os.path.join(
    cur_dir, "Intersection", "intersection.sumocfg")

traci.start(["sumo", "-c", sumo_config_path])

# Create the GCN model
# Adjust input and output dimensions as needed
gcn_model = GCNModel(in_channels=7, hidden_channels=32, out_channels=16)
...
for i in range(300):
    ...
    if G.number_of_edges() > 0:
        ...
        node_embeddings = gcn_model(x, edge_index)
...
with open('output.csv', mode='w', newline='') as file:
    ...
    for id in G.nodes():
        ...
        writer.writerow([id, speed[0], pos[0], pos[1], lane])
